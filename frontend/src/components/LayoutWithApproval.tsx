/**
 * Layout mejorado con soporte para badge de usuarios pendientes de aprobación
 */
...
export default function LayoutWithApproval() {
  ...
  // Query para obtener contador de usuarios pendientes
  const { data: pendingCount = 0 } = useQuery({
    queryKey: ['pendingCount'],
    queryFn: () => approvalService.getPendingCount(),
    refetchInterval: 30000, // Refetch cada 30 segundos
    enabled: canManageUsers, // Solo si puede gestionar usuarios
  });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Main Content */}
      <div
        className={`transition-all duration-200 ${sidebarOpen ? 'md:ml-64' : 'ml-0'}`}
      >
        {/* Top Bar */}
        <header className="sticky top-0 z-40 flex items-center justify-between h-16 px-6 bg-white border-b border-gray-200">
          ...
          <div className="flex items-center space-x-4">
            {/* Pending Approvals Badge (si hay) */}
            {pendingCount > 0 && canManageUsers && (
              <Link
                to="/approvals"
              ...
              >
                ...
                <span className="text-sm font-medium text-orange-700">
                  {pendingCount} usuario{pendingCount !== 1 && 's'} pendiente{pendingCount !== 1 && 's'}
                </span>
              </Link>
            )}
